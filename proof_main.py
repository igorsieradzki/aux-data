...
def main(argv=None):
    ...
    # dataset args
    parser.add_argument('--datasets', default=['cifar10_C'], type=str, choices=list(dataset_config.keys()),
    ...
    # model args
    # training args
    parser.add_argument('--approach', default='proof', type=str, choices=['proof', 'aux.proof'],
                        help='Learning approach used (default=%(default)s)', metavar="APPROACH")
    parser.add_argument('--network', default='resnet32', type=str, choices=allmodels,
                        help='Network architecture used (default=%(default)s)', metavar="NETWORK")
    # training args
    parser.add_argument('--nepochs', default=20, type=int, required=False,
    ...
    # gridsearch args
    parser.add_argument('--gridsearch-tasks', default=5, type=int,
    ...
    if args.gridsearch_tasks > 0:
        ...
        if 'aux' in args.approach:
            Appr_finetuning = getattr(importlib.import_module(name='approach.aux.finetuning'), 'Appr')
        else:
    ...
    # Args -- Network
    net_cls = getattr(importlib.import_module(name='networks'), args.network)
    appr = Appr(net_cls, device, **appr_kwargs)

    # GridSearch
    if args.gridsearch_tasks > 0:
        ft_kwargs = {**base_kwargs, **dict(logger=logger,
                                           exemplars_dataset=GridSearch_ExemplarsDataset(transform, class_indices))}
        appr_ft = Appr_finetuning(net_cls, device, **ft_kwargs)
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        if t < args.gridsearch_tasks:
            ...
            print('LR GridSearch')
            init_model = net_cls(pretrained=False)
            net = LLL_Net(init_model, remove_existing_head=True)
            for _ in range(t + 1):
                net.add_head(2)
            net.to(device)
            best_ft_acc, best_ft_lr = gridsearch.search_lr(net, t, trn_loader[t], val_loader[t])
            # Apply to approach
            appr.lr = best_ft_lr

...
class Appr(Aux_Inc_Learning_Appr):
    ...
    def train_epoch(self, t, trn_loader):
        ...
        with GradCAM(self.model_old, self.gradcam_layer) as gradcam_old:
            for images, targets in trn_loader:
                ...
                loss = self.binary_criterion(t, outputs, targets.to(self.device), outputs_old, attmap, attmap_old)
                # Backward
    ...
    def binary_criterion(self, t, outputs, targets, outputs_old=None, attmap=None, attmap_old=None):
        """Returns the loss value"""
        loss = 0
        if t > 0 and outputs_old is not None:
            # Knowledge distillation loss for all previous tasks

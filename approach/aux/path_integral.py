...
class Appr(Aux_Inc_Learning_Appr):
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            if any(targets < 0):
                ...
                _targets = targets - self.model.task_offset[t]

                _targets[neg_ids] = 0
                # one-hot encode target for binary loss
                one_hot_target = torch.nn.functional.one_hot(_targets, num_classes=self.model.heads[t].out_features).float()
                one_hot_target[neg_ids] = torch.zeros(one_hot_target.shape[1]).to(self.device).float()
            else:
                _targets = targets - self.model.task_offset[t]
                one_hot_target = torch.nn.functional.one_hot(_targets.to(self.device), num_classes=self.model.heads[t].out_features).float()
            ...
            if len(self.exemplars_dataset) == 0:
                ...
                loss = torch.nn.functional.binary_cross_entropy_with_logits(outputs[t], one_hot_target)
            else:
                ...
                loss = torch.nn.functional.binary_cross_entropy_with_logits(torch.cat(outputs, dim=1), one_hot_target)
    ...
    def binary_criterion(self, t, outputs, targets):
        ...
        if len(self.exemplars_dataset) > 0:
            return loss + torch.nn.functional.binary_cross_entropy_with_logits(torch.cat(outputs, dim=1), one_hot_target)

        return loss + torch.nn.functional.binary_cross_entropy_with_logits(outputs[t], one_hot_target)

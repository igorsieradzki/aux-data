...
class Appr:
    ...
    def criterion(self, t, outputs, targets, eval=False):
        ...
        if eval:
            return torch.nn.functional.cross_entropy(outputs[t], targets - self.task_offset[t])
        else:
            return torch.nn.functional.cross_entropy(outputs[0], targets - self.task_offset[t])

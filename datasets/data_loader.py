...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, validation=.1, negative_frac=1.):
...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None,
    ...
    for task in range(num_tasks):
        ...
        all_data[task]['trn']['y'] = list(map(lambda y: y + offset if y != -1 else -1, all_data[task]['trn']['y']))
